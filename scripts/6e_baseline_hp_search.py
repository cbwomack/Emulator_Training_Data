...
"""
Phase 0, Stage 6e Part B (moved up from Phase 1's reviewer-response work):
independently tune the baseline emulator's own hyperparameters - K
(training length), lr, weight_decay - the knobs `train_baseline_emulator`/
`evaluate_baseline_over_multiple_tests` already expose. This directly
targets reviewer Point 5: the baseline currently "utilizes an identical
architecture, feature set, and training hyperparameters... as" the
optimized emulator (K=400, lr=5e-2, weight_decay=1e-2, copied verbatim from
the inner-loop's own settings) - i.e. it was never independently tuned.
Architecture (hidden_sizes) is deliberately out of scope here, covered
separately by SIf_sensitivity.ipynb.

Note (post-hoc, per user direction): this search's winner used K=1600
(the search found K=1600 dominates every other value at any lr/weight_decay
tried - see REVISIONS.md), but the baseline actually deployed for Stage 6e
Part C's gap-persistence check is held at K=400 to match the optimized
emulator's fixed K_inner=400, keeping that specific comparison
apples-to-apples in training budget. The chosen lr/weight_decay
(0.1329/0.0) are assumed to transfer from K=1600 to K=400 without a
separate K=400-only search - not re-verified independently.

Method: single-phase random search (unlike Stage 0b's cheap/expensive
split - there's no cheaper proxy here, every run already is the
full-fidelity evaluation since baseline training has no outer loop).
N candidate (K, lr, weight_decay) configs x 5 seeds, each evaluated via the
existing evaluate_baseline_over_multiple_tests on the real CO2-only
Tier1/Tier2/DECK/CS3/All eval sets. Scored by the SAME Fig-4 weighting
already used elsewhere (Tier1:7, Tier2:5, DECK:2, CS3:2 - 'All' excluded
to avoid double-counting, since it's the union of the other four).
...
compile time), not locally - same reasoning as Stage 0b.

Usage:
    python 6e_baseline_hp_search.py --mode search-one --config-idx 0 --seed 0
    python 6e_baseline_hp_search.py --mode search --n-configs 60 --seeds 0,1,2,3,4  # single-machine convenience
    python 6e_baseline_hp_search.py --mode collect
    python 6e_baseline_hp_search.py --mode finalize
    python 6e_baseline_hp_search.py --mode report
"""
import os
import sys
import csv
...
N_CONFIGS_DEFAULT = 60
SEEDS_DEFAULT = [0, 1, 2, 3, 4]


def sample_config(rng):
    return {
        "K": int(rng.choice([100, 200, 400, 800, 1600])),
        "lr": float(np.exp(rng.uniform(np.log(0.01), np.log(0.2)))),
        "weight_decay": float(rng.choice([0.0, 1e-3, 3e-3, 1e-2, 3e-2, 1e-1])),
    }
...
def finalize():
    ...
    out_path = OUT_DIR / "best_baseline_config.json"
    ...
    print(f"  (current/original baseline: K=400, lr=0.05, weight_decay=0.01)")


def report():
    for kind, path in [("search results", OUT_DIR / "search_results.csv"),
                        ("best config", OUT_DIR / "best_baseline_config.json")]:
    ...
    best_path = OUT_DIR / "best_baseline_config.json"
