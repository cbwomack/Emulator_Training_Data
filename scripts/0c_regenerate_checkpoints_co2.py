...
Usage:
    python 0c_regenerate_checkpoints_co2.py                # run every group
    python 0c_regenerate_checkpoints_co2.py --group H-ext  # run just one
"""
import os
import sys
import json
...
def main():
    ...
    parser.add_argument("--seed", type=int, default=0,
                         help="Seed for params0/baseline init (default 0, matches original checkpoints)")
    ...
    baseline_save_path = f"{CHECKPOINT_DIR}/baseline_co2_only.pkl" if write_baseline else None

    setup = utils_inverse.run_inverse_experiment_setup(
        AGENTS, ACTIVE_AGENTS, mode=MODE,
        CS3=True, DAMIP=False, GeoMIP=False,
        idx_demo=None, seed=args.seed,
